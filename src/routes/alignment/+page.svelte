<script lang="ts">
	...
	$: planetEntries = chartData
		? (Object.entries(chartData.planets) as Array<[string, number]>)
		: [];
</script>

<div
	class="min-h-screen p-6"
	style="background: linear-gradient(135deg, #0B0724 0%, #17133A 100%);"
>
	<div class="max-w-7xl mx-auto">
		...
		<div class="mb-8">
			<a
				href="/"
				class="inline-flex items-center text-lg font-semibold transition-all duration-200 hover:opacity-80"
				style="color: #C6A7FF;"
			>
				← Back to Home
			</a>
		...
		<div class="text-center mb-12">
			...
			<p class="text-2xl mb-6" style="color: #C6A7FF;">
				Discover your birth chart and planetary positions
			</p>

			<!-- Info Banner -->
			<div
				class="max-w-4xl mx-auto card-surface p-6 rounded-2xl"
				style="background: linear-gradient(135deg, rgba(123, 97, 255, 0.1), rgba(255, 78, 219, 0.1)); border: 2px solid rgba(123, 97, 255, 0.3);"
			>
				<div class="flex items-start gap-4">
					...
					<div class="text-left">
						...
						<p class="text-sm leading-relaxed" style="color: #C6A7FF;">
							Your birth chart is a snapshot of the sky at the exact moment and location of your
							birth. It reveals the positions of planets, your rising sign (Ascendant), and how
							celestial energies influence your personality, relationships, and life path. Hover
							over the info icons (ℹ️) throughout the page for helpful tips!
						</p>
					</div>
				</div>
			</div>
				...
				<div class="space-y-6">
					...
					<div>
						<div class="flex items-center gap-2 mb-3">
							<label for="date" class="text-lg font-semibold" style="color: #C6A7FF;"
								>Birth Date</label
							>
							<InfoTooltip
								text="Your birth date determines your Sun sign and all planetary positions. This is the foundation of your birth chart and reveals your core personality traits and life path."
							/>
						</div>
						<input
							id="date"
					...
					<div>
						<div class="flex items-center gap-2 mb-3">
							<label for="time" class="text-lg font-semibold" style="color: #C6A7FF;"
								>Birth Time</label
							>
							<InfoTooltip
								text="Your exact birth time is crucial for calculating your Ascendant (rising sign) and house placements. Even a few minutes can change your rising sign! If unknown, use 12:00 PM as an approximation."
							/>
						</div>
						<input
							id="time"
					...
					<div>
						<div class="flex items-center justify-between mb-3">
							<div class="flex items-center gap-2">
								...
								<InfoTooltip
									text="Your birth location determines the time zone and affects house calculations. The location where you were born is important, not where you live now. We'll try to detect your current location, or you can enter coordinates manually."
								/>
							</div>
							{#if locationLoading}
						...
						<div class="grid grid-cols-2 gap-4">
							<div>
								<div class="flex items-center gap-2 mb-2">
									<label for="latitude" class="text-sm font-semibold" style="color: #C6A7FF;"
										>Latitude</label
									>
									<InfoTooltip
										text="Latitude is your north-south position on Earth. Positive values are north of the equator (e.g., New York is 40.71), negative values are south (e.g., Sydney is -33.87)."
									/>
								</div>
								<input
									id="latitude"
							...
							<div>
								<div class="flex items-center gap-2 mb-2">
									<label for="longitude" class="text-sm font-semibold" style="color: #C6A7FF;"
										>Longitude</label
									>
									<InfoTooltip
										text="Longitude is your east-west position on Earth. Negative values are west of the Prime Meridian (e.g., New York is -74.00), positive values are east (e.g., Tokyo is 139.69)."
									/>
								</div>
								<input
									id="longitude"
			...
			<div class="lg:col-span-2">
				...
				{:else if chartData}
					...
					<div class="card-surface p-8 rounded-2xl mb-8">
						<div class="flex items-center gap-2 mb-6">
							...
							<InfoTooltip
								text="These show where each planet was located in the zodiac at your birth time. Each planet represents different aspects of your personality and life."
							/>
						</div>
						<div class="grid grid-cols-2 gap-4">
							{#each planetEntries as [planet, longitude]}
								...
								>
									<div class="flex items-center justify-between mb-2">
										...
										<span class="text-sm font-semibold" style="color: #C6A7FF;"
											>{planet.charAt(0).toUpperCase() + planet.slice(1)}</span
										>
									</div>
									<p style="color: #EDEBFF;">
										<span style="color: #FF4EDB;">{getZodiacEmoji(getZodiacSign(longitude))}</span>
										{getZodiacSign(longitude)}
										{getZodiacDegree(longitude)}°
									</p>
					...
					<div class="card-surface p-8 rounded-2xl mb-8">
						<div class="flex items-center gap-2 mb-6">
							...
							<InfoTooltip
								text="Ascendant (Rising Sign) is how you appear to others. Midheaven (MC) represents your career path and public image. Both are calculated from your exact birth time and location."
							/>
						</div>
						<div class="grid grid-cols-2 gap-4">
							<div
							...
							>
								...
								<p style="color: #EDEBFF;">
									<span style="color: #FF4EDB;"
										>{getZodiacEmoji(getZodiacSign(chartData.ascendant))}</span
									>
									{getZodiacSign(chartData.ascendant)}
									{getZodiacDegree(chartData.ascendant)}°
								</p>
							</div>
							<div
								class="p-4 rounded-lg"
							...
							>
								...
								<p style="color: #EDEBFF;">
									<span style="color: #FF4EDB;"
										>{getZodiacEmoji(getZodiacSign(chartData.midheaven))}</span
									>
									{getZodiacSign(chartData.midheaven)}
									{getZodiacDegree(chartData.midheaven)}°
								</p>
							</div>
						</div>
					</div>
					...
					{#if chartData.aspects && chartData.aspects.length > 0}
						<div class="card-surface p-8 rounded-2xl">
							<div class="flex items-center gap-2 mb-6">
								...
								<InfoTooltip
									text="Aspects are angular relationships between planets. They show how planetary energies interact: harmoniously (trine, sextile) or with tension (square, opposition)."
								/>
							</div>
							<div class="space-y-3">
									...
									>
										<div class="flex items-center justify-between">
											<span style="color: #EDEBFF;">
												{planetEmojis[aspect.planet1] || '●'}
												{aspect.planet1}
												<span style="color: #FF4EDB;"> {aspect.type} </span>
												{planetEmojis[aspect.planet2] || '●'}
												{aspect.planet2}
											</span>
				...
				{:else}
					...
					<div class="space-y-6">
						<!-- Privacy Disclaimer -->
						<div
							class="card-surface p-6 rounded-2xl privacy-banner"
							style="background: linear-gradient(135deg, rgba(77, 242, 176, 0.25), rgba(67, 217, 157, 0.2)); border: 3px solid #4DF2B0; box-shadow: 0 0 30px rgba(77, 242, 176, 0.4);"
						>
							<div class="flex items-start gap-4">
								...
								<div class="text-left">
									<h3
										...
										style="color: #4DF2B0; text-shadow: 0 0 10px rgba(77, 242, 176, 0.5);"
									>
										🛡️ Your Privacy is 100% Protected
									</h3>
									<p class="text-base leading-relaxed font-medium" style="color: #EDEBFF;">
										✅ All calculations are performed <strong style="color: #4DF2B0;"
											>locally in your browser</strong
										><br />
										✅ We <strong style="color: #4DF2B0;">never store, transmit, or collect</strong>
										any personal information<br />
										✅ Your birth data and location details remain
										<strong style="color: #4DF2B0;">completely private</strong>
									</p>
								</div>
							</div>
						</div>
						...
						<!-- Educational Content -->
						<div class="card-surface p-8 rounded-2xl">
							<h3 class="text-2xl font-bold mb-6 text-center" style="color: #EDEBFF;">
							...
							<div class="space-y-6">
								...
								<div>
									<h4 class="text-lg font-bold mb-4" style="color: #C6A7FF;">
										♈ The Zodiac Signs
									</h4>
									...
									<p class="text-xs mt-3 text-center" style="color: #B3A9C7;">
										Each planet falls into a zodiac sign, coloring how that planet's energy
								...
								<div class="text-center pt-4">
									<p class="text-lg" style="color: #C6A7FF;">
										👈 Fill in your birth information to discover your unique cosmic blueprint!
									</p>
								</div>
							</div>
						</div>
