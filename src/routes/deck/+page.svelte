	...
	async function loadAllCards() {
		try {
			...
			allCards = cardFiles.map((filename) => {
				...
				else if (cardName.includes('of Tide'))
					suit = 'Tides'; // Handles both "Tide" and "Tides"
...
<div class="min-h-screen p-8">
	<div class="max-w-7xl mx-auto">
		...
		<div class="text-center mb-12">
			...
			<p class="text-sm mt-2" style="color: #B3A9C7;">
				Original designs and classic wisdom combined
			</p>
		</div>

		<!-- Celestia Arcana Deck -->
		<div
			class="mb-16 card-surface p-8 rounded-2xl"
			style="border: 2px solid rgba(123, 97, 255, 0.3); background: linear-gradient(135deg, rgba(123, 97, 255, 0.05), rgba(255, 78, 219, 0.05));"
		>
					...
					<div class="mb-12">
						<div class="flex items-center gap-3 mb-6">
							...
							<span
								...
								style="background-color: rgba(123, 97, 255, 0.2); color: #C6A7FF;"
							>
						...
						<div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3">
							{#each section.cards as card (card.id)}
								...
								>
									<div
										class="w-20 h-28 rounded-lg border-2 overflow-hidden mb-2 transition-all group-hover:shadow-lg"
										style="border-color: #7B61FF; box-shadow: 0 5px 10px rgba(123, 97, 255, 0.2); group-hover:border-color: #FF4EDB;"
									>
										<img
									...
									<p style="color: #C6A7FF;" class="text-xs font-semibold text-center line-clamp-2">
										{card.name}
									</p>
								</button>
							{/each}
...
{#if showModal && selectedCard}
	...
	>
		...
		>
			...
			<!-- Card Image -->
			<div
				class="w-48 h-72 rounded-lg border-2 overflow-hidden mb-6"
				style="border-color: #7B61FF; box-shadow: 0 10px 15px -3px rgba(123, 97, 255, 0.3);"
			>
				<img
			...
			<h2
				...
				style="color: #EDEBFF; background: linear-gradient(135deg, #7B61FF, #FF4EDB); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;"
			>
				{selectedCard.name}
			</h2>
			...
			<div class="w-full space-y-6">
				...
				<div
					...
					style="background: linear-gradient(135deg, rgba(123, 97, 255, 0.1), rgba(123, 97, 255, 0.05)); border-left: 4px solid #7B61FF;"
				>
				...
				<div
					class="p-6 rounded-lg"
					style="background: linear-gradient(135deg, rgba(255, 78, 219, 0.1), rgba(255, 78, 219, 0.05)); border-left: 4px solid #FF4EDB;"
				>
			...
			<div class="text-center mt-8">
				<p class="text-sm" style="color: #B3A9C7;">Press ESC or click outside to close</p>
			</div>
		</div>
	</div>
{/if}
...
<!-- Toast Notification -->
<Toast visible={toastVisible} message={toastMessage} x={toastX} y={toastY} />
