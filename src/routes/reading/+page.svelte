<script lang="ts">
	...
	import ShuffleOverlay, {
		buildShuffleCards,
		type ShuffleCard
	} from '$lib/components/ShuffleOverlay.svelte';
...
<div
	class="min-h-screen p-8 lg:p-20 overflow-x-hidden"
...
>
	<div class="max-w-6xl mx-auto w-full px-6 lg:px-16">
		...
		<div class="text-center mb-20">
			...
			<h1
				class="text-7xl lg:text-8xl font-bold mb-4"
				style="background: linear-gradient(135deg, #7B61FF, #FF4EDB, #4DF2B0); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; text-shadow: 0 0 30px rgba(123, 97, 255, 0.3);"
			>
				Celestial Reading
			</h1>
			<p class="text-xl lg:text-2xl mb-2" style="color: #C6A7FF;">
				Combine tarot wisdom with your birth chart
			</p>
			<p class="text-sm" style="color: #B3A9C7;">
				Discover cosmic guidance tailored to your unique astrological profile
			</p>
		</div>
		...
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-12 overflow-hidden">
				...
				>
					...
					<div class="space-y-6">
						...
						<div>
							<label
								for="question"
								class="block text-lg font-semibold mb-3 flex items-center gap-2"
								style="color: #C6A7FF;"
							>
								<span>❓</span>
								...
								<button
									...
									on:click={() =>
										scheduleToast(
											'💡 Questions give your reading focus and direction, helping the cards speak to what you need. They\'re optional though — you can explore broadly ("What do I need to know?") or simply see what unfolds without a specific question. The cards will reveal what\'s needed either way.'
										)}
							...
							<div class="grid grid-cols-3 gap-3">
								<div>
									<label
										...
										style="color: #C6A7FF;">Month</label
									>
									<input
										id="month"
										type="number"
								...
								<div>
									<label for="day" class="block text-sm font-semibold mb-2" style="color: #C6A7FF;"
										>Day</label
									>
									<input
										id="day"
								...
								<div>
									<label for="year" class="block text-sm font-semibold mb-2" style="color: #C6A7FF;"
										>Year</label
									>
									<input
										id="year"
						...
						<div>
							<div class="flex items-center justify-between mb-3">
								<span
									class="block text-lg font-semibold flex items-center gap-2"
									style="color: #C6A7FF;"
								>
									<span>📍</span>
									Your Location
							...
							<div class="grid grid-cols-2 gap-4">
								<div>
									<label
										for="latitude"
										class="block text-sm font-semibold mb-2"
										style="color: #C6A7FF;">Latitude</label
									>
									<input
										id="latitude"
										type="number"
								...
								<div>
									<label
										for="longitude"
										class="block text-sm font-semibold mb-2"
										style="color: #C6A7FF;">Longitude</label
									>
									<input
										id="longitude"
										type="number"
						...
						<button
							...
							style="background: linear-gradient(135deg, #7B61FF, #FF4EDB); color: #EDEBFF; opacity: {loading
								? 0.6
								: 1}; cursor: {loading
								? 'not-allowed'
								: 'pointer'}; box-shadow: 0 10px 30px rgba(123, 97, 255, 0.3);"
						>
						...
						{#if error}
							<div
								class="p-5 rounded-lg text-lg"
								style="background-color: rgba(255, 78, 219, 0.1); border-left: 4px solid #FF4EDB; color: #EDEBFF;"
							>
								{error}
							</div>
						{/if}
					...
					<div class="flex flex-col items-center justify-center min-h-96">
						...
						{#if isShuffling || (isDealing && dealtCards.length === 0)}
							<div
								...
								style="background: linear-gradient(135deg, #7B61FF, #FF4EDB); border-color: #C6A7FF; box-shadow: 0 20px 40px rgba(123, 97, 255, 0.3); animation: {isShuffling
									? 'shuffle 1.2s ease-in-out'
									: 'none'};"
							>
						...
						{#if dealtCards.length > 0}
							<div class="w-full">
								<h3 class="text-2xl font-bold mb-6 text-center" style="color: #C6A7FF;">
									Your Cards
								</h3>
									...
									{#each dealtCards as dealtCard, index (dealtCard.index)}
										...
										>
											...
											<div class="text-center mb-3">
												<p class="text-sm font-semibold" style="color: #C6A7FF;">
													{dealtCard.position}
												</p>
											</div>
											...
											<p class="text-lg font-semibold text-center" style="color: #EDEBFF;">
												{dealtCard.card.name}
											</p>
											...
											{#if reading.analysis && reading.analysis.cards}
												{@const cardMeaning = (reading.analysis.cards as CardInterpretation[]).find(
													(c) => c.name === dealtCard.card.name
												)}
												{#if cardMeaning}
													<div
			...
			<!-- Reading Display -->
			<div
				class="lg:col-span-3 overflow-x-hidden"
				style="background: linear-gradient(135deg, rgba(77, 242, 176, 0.08), rgba(123, 97, 255, 0.12)); border: 2px solid rgba(123, 97, 255, 0.3); border-radius: 2rem; padding: 2.5rem 1.5rem; backdrop-filter: blur(10px);"
			>
				{#if reading}
					<div class="flex items-center justify-between mb-8">
						...
						<button
							...
							style="background: linear-gradient(135deg, #7B61FF, #FF4EDB); color: #EDEBFF; opacity: {isSpeaking
								? 0.7
								: 1}; cursor: pointer; box-shadow: 0 5px 15px rgba(123, 97, 255, 0.2);"
						>
					...
					{#if drawnCards.length > 0}
						<div
							...
							class="mb-12 pb-8 border-b-2"
							style="border-color: rgba(123, 97, 255, 0.3);"
						>
							...
							<div class="grid grid-cols-3 gap-4 max-w-4xl mx-auto">
								{#each drawnCards as card, index (index)}
									...
									>
										...
										<p class="text-xs font-semibold text-center" style="color: #EDEBFF;">
											{card.card.name}
										</p>
										{#if card.reversed}
					...
					<div class="space-y-8">
						...
						{#if question}
							<div
								...
								style="background: linear-gradient(135deg, rgba(255, 78, 219, 0.15), rgba(123, 97, 255, 0.1)); border: 2px solid #FF4EDB;"
							>
						...
						{#if reading.combinedReading}
							<div
								class="p-8 rounded-xl mb-8"
								style="background: linear-gradient(135deg, rgba(123, 97, 255, 0.15), rgba(255, 78, 219, 0.1)); border: 2px solid #7B61FF; box-shadow: 0 10px 30px rgba(123, 97, 255, 0.2);"
							>
								...
								<div class="mt-6 pt-4 border-t" style="border-color: rgba(123, 97, 255, 0.3);">
									<p class="text-sm italic" style="color: rgba(237, 235, 255, 0.7);">
										Disclaimer: This reading is for entertainment purposes only. Tarot and astrology
						...
						{#if showTraditionalReading && reading.reading}
							<div
								class="p-7 rounded-xl mb-8"
								style="background: linear-gradient(135deg, rgba(77, 242, 176, 0.12), rgba(123, 97, 255, 0.08)); border: 2px solid rgba(77, 242, 176, 0.4); box-shadow: 0 8px 24px rgba(77, 242, 176, 0.15);"
							>
				...
				{:else if !loading}
					<div class="text-center py-20">
						...
						<p class="text-2xl" style="color: #B3A9C7;">
							Fill in your information and click "Get Your Reading" to begin
						</p>
					</div>
				{:else}
					<div class="text-center py-20">
