...
function findPythonExecutable(): string {
	...
	const errorMsg =
		'Python not found. Please install Python 3.6+ and ensure it is in your PATH.\n' +
		'Tried: ' +
		candidates.join(', ') +
		'\n' +
		'Current PATH: ' +
		(process.env.PATH || 'not set');
...
function executePythonScript(payload: AstroTarotRequest): Promise<PythonOutput> {
	return new Promise((resolvePromise, reject) => {
		...
		const args = [
			...
			'--temperature',
			String(payload.temperature || 0.2),
			'--num-predict',
			String(payload.num_predict || 1500),
		...
		pythonProcess.on('close', (code: number | null) => {
			...
			try {
				...
				if (sentinelStart !== -1 && sentinelEnd > sentinelStart) {
					jsonText = stdout.slice(sentinelStart + '<<<JSON_BEGIN>>>'.length, sentinelEnd).trim();
				} else {
