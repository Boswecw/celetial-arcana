...
async function generateReading(body: any, analysis: any, ragContext: string): Promise<string> {
...
function generateMockReading(body: any, analysis: any): string {
	const cardList = body.draw
		.map((d: any) => `${d.card.name}${d.reversed ? ' (reversed)' : ''}`)
		.join(', ');
