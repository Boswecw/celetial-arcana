...
class AITrainer {
	 ...
	 */
	generateOptimizedPrompt(userZodiac?: string, cards?: string[], themes?: string[]): string {
...
- Consistent: reference the provided deck meanings and astrological lore`;
		...
		if (userZodiac) {
			...
			if (userPrefs && userPrefs.ratings.length > 0) {
				const avgRating =
					userPrefs.ratings.reduce((a: number, b: number) => a + b, 0) / userPrefs.ratings.length;
...
Focus on the dynamic interplay between these cards and their collective message.`;
		...
		if (themes && themes.length > 0) {
			const topThemes = themes
				...
				.sort(
					(a, b) =>
						meanOf(this.trainingData.themeWeights.get(b)) -
						meanOf(this.trainingData.themeWeights.get(a))
				)
				.slice(0, 2);
...
These themes resonate strongly with users. Weave them throughout the reading.`;
	...
	getImprovementRecommendations(): string[] {
		...
		if (failedPatterns.length > 0) {
			recommendations.push(`Avoid these patterns: ${failedPatterns.map((p) => p[0]).join(', ')}`);
		}
		...
		if (topThemes.length > 0) {
			recommendations.push(`Focus on these themes: ${topThemes.map((t) => t[0]).join(', ')}`);
		}
		...
		return {
			...
			averageRating:
				this.feedbackHistory.length > 0
					? this.feedbackHistory.reduce((sum, f) => sum + f.rating, 0) / this.feedbackHistory.length
					: 0,
	...
	importTrainingData(data: any): void {
		...
		this.trainingData.userPreferences = new Map(Object.entries(data.userPreferences || {}));
