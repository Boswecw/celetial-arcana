...
export function applyGuardrails(content: string, userQuestion?: string): GuardrailResult {
	...
	const isSensitive =
		hasSensitiveContent(content) || (userQuestion && hasSensitiveContent(userQuestion));
	if (isSensitive) {
		warnings.push(
			'This reading addresses sensitive topics. Please seek professional support if needed.'
		);
	...
	if (content.toLowerCase().includes('health') || content.toLowerCase().includes('medical')) {
		warnings.push(
			'This reading mentions health topics. Consult a healthcare provider for medical concerns.'
		);
	...
	if (
		...
		warnings.push(
			'This reading touches on mental health. Consider speaking with a mental health professional.'
		);
