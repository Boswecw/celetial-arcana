...
<div class="card-surface p-8 rounded-2xl mt-8">
	<h3 class="text-2xl font-bold mb-6" style="color: #C6A7FF;">✨ How was this reading?</h3>

	{#if submitted}
		<div class="text-center py-6">
			<p class="text-xl" style="color: #7B61FF;">Thank you! Your feedback helps us improve. 🙏</p>
		</div>
	{:else}
		<div class="space-y-6">
			<!-- Rating Stars -->
			...
			{#if errorMessage}
				<p class="text-sm" role="alert" style="color: #FF6B6B;">
					{errorMessage}
				</p>
			{/if}
			...
			<button
				...
				style="background-color: #7B61FF; color: #EDEBFF; opacity: {loading || rating === 0
					? 0.6
					: 1}; cursor: {loading || rating === 0 ? 'not-allowed' : 'pointer'};"
			>
