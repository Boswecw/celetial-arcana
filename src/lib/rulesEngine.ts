...
function extractThemes(cards: CardInterpretation[], aspects: AspectInterpretation[]): string[] {
...
function generateInterpretations(
	...
	if (aspects.length > 0) {
		interpretations.push(
			`Key aspect: ${aspects[0].type} between ${aspects[0].planet1} and ${aspects[0].planet2}`
		);
	}
